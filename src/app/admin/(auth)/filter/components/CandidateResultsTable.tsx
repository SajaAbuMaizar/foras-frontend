...
import { ChevronDown, ChevronUp, Phone, MapPin, Languages, Award, Car } from "lucide-react";
...
export default function CandidateResultsTable({ 
  ...
  const handleSort = (column: string) => {
    const newDirection = currentSort.by === column && currentSort.direction === 'asc' ? 'desc' : 'asc';
                    ...
                    <td colSpan={6} className="px-6 py-4">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          {candidate.skills.length > 0 ? (
                            ...
                            <p className="text-sm text-gray-500">لا توجد مهارات</p>
                          )}
                        </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h4 className="font-medium text-gray-900">رخص القيادة</h4>
                          </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h4 className="font-medium text-gray-900">اللغات</h4>
                          </div>
                          {candidate.languages.length > 0 ? (
                            ...
                            <p className="text-sm text-gray-500">لا توجد لغات</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <span className="font-medium text-gray-600">يحتاج مساعدة: </span>
                            <span className={candidate.needsHelp ? 'text-orange-600' : 'text-green-600'}>
                              {candidate.needsHelp ? 'نعم' : 'لا'}
                            </span>
                          </div>
                          <div>
                            <span className="font-medium text-gray-600">رقم المرشح: </span>
                            <span className="text-gray-900">#{candidate.id}</span>
                          </div>
                        </div>
                      </div>
                    </td>
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  عرض{' '}
                  <span className="font-medium">{candidates.pageNumber * candidates.pageSize + 1}</span>
                  {' '}إلى{' '}
                  <span className="font-medium">
                    {Math.min((candidates.pageNumber + 1) * candidates.pageSize, candidates.totalElements)}
                  </span>
                  {' '}من{' '}
                  <span className="font-medium">{candidates.totalElements}</span>
                </p>
              </div>
              <div>
                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  ...
                  {/* Page Numbers */}
                  {[...Array(Math.min(5, candidates.totalPages))].map((_, index) => {
                    const pageNum = Math.max(0, Math.min(
                      candidates.pageNumber - 2 + index,
                      candidates.totalPages - 1
                    ));
                    
                    if (pageNum < 0 || pageNum >= candidates.totalPages) return null;
